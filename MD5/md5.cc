...
MD5::MD5(unsigned char* str, size_t str_size)
{
	init();
	update(str, str_size);
	finalize();
...
void MD5::raw_digest(std::string &output)
{
	if(!finalized){
		std::cerr << "MD5::raw_digest:  Can't get digest if you haven't "<<
			"finalized the digest!" <<std::endl;
	...
	output.assign((char const*)digest, 16);
}
...
void MD5::hex_digest(char *output)
{
	if (!finalized){
		std::cerr << "MD5::hex_digest:  Can't get digest if you haven't "<<
			"finalized the digest!" <<std::endl;
...
void MD5::hex_digest(std::string &output)
{
	if (!finalized){
		std::cerr << "MD5::hex_digest:  Can't get digest if you haven't "<<
			"finalized the digest!" <<std::endl;
